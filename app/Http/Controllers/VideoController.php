         ...
         return response()->json(['status'=>'success','comment-reply' => $success], $this-> successStatus); 
            ...
            public function updateReply(Request $request)
            {
               ...
               if ($comment) {
                     $comment->comment = $request->comment;
                     $comment->save();
                     return response()->json(['status'=>'success','comment-reply' => 'Comment Reply Updated Successfully'], $this-> successStatus); 
            ...
            public function like_reply(Request $request){
               Comment_reply::where('id',$request->id)->update(['status'=>1]);
               return response()->json(['status'=>'success','comment-reply' => 'Comment Reply Like Successfully'], $this-> successStatus); 

         }

         public function unlike_reply(Request $request){
            Comment_reply::where('id',$request->id)->update(['status'=>0]);
            return response()->json(['status'=>'success','comment-reply' => 'Comment Reply Unlike Successfully'], $this-> successStatus); 
         }
